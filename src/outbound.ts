...
export async function sendText(ctx: OutboundContext): Promise<OutboundResult> {
  ...
  if (replyToId) {
    ...
    if (!limitCheck.allowed) {
      ...
      if (limitCheck.shouldFallbackToProactive) {
        console.warn(
          `[qqbot] sendText: 被动回复不可用，降级为主动消息 - ${limitCheck.message}`,
        );
        ...
        // 不应该发生，但作为保底
        console.error(
          `[qqbot] sendText: 消息回复被限流但未设置降级 - ${limitCheck.message}`,
        );
        return {
          channel: "qqbot",
  ...
  if (!replyToId) {
    if (!text || text.trim().length === 0) {
      console.error(
        "[qqbot] sendText error: 主动消息的内容不能为空 (text is empty)",
      );
      return {
        channel: "qqbot",
    ...
    if (fallbackToProactive) {
      console.log(
        `[qqbot] sendText: [降级] 发送主动消息到 ${to}, 内容长度: ${text.length}`,
      );
    } else {
      console.log(
        `[qqbot] sendText: 发送主动消息到 ${to}, 内容长度: ${text.length}`,
      );
    }
  }
...
export async function sendMedia(
  ...
  if (isLocalPath) {
    ...
    console.log(
      `[qqbot] sendMedia: unsupported media format: ${mediaUrl.slice(0, 50)}`,
    );
    return {
      channel: "qqbot",
    ...
    console.log(
      `[qqbot] sendMedia: sending Base64 image (length: ${mediaUrl.length})`,
    );
  } else {
    console.log(
      `[qqbot] sendMedia: sending image URL: ${mediaUrl.slice(0, 80)}...`,
    );
  }
...
export async function sendCronMessage(
  ...
  console.log(
    `[qqbot] sendCronMessage: to=${to}, message length=${message.length}`,
  );
  ...
  if (cronResult.isCronPayload) {
    if (cronResult.error) {
      console.error(
        `[qqbot] sendCronMessage: cron payload decode error: ${cronResult.error}`,
      );
      return {
        channel: "qqbot",
    ...
    if (cronResult.payload) {
      ...
      console.log(
        `[qqbot] sendCronMessage: decoded cron payload, targetType=${payload.targetType}, targetAddress=${payload.targetAddress}`,
      );
